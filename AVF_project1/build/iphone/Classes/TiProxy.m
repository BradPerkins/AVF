...
//Rare exceptions to indicate a bug in the _weather2 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_weather2 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_weather2]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.weather2.module"
