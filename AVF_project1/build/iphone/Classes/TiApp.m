...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kweather2UserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kweather2UserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a weather2 app using the node.js CLI
